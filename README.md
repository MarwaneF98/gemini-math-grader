...
![Gemini API](https://img.shields.io/badge/API-Gemini%203.5%20Flash-8A2BE2?logo=google&logoColor=white)
...
This script acts like a virtual red pen. It analyzes a photo of a math paper, evaluates each step of the equations for accuracy, and draws feedback exactly where it belongs. It uses a strict grading system to score overall problems, but still adds green checkmarks for correct intermediate steps, red bounding boxes with constructive feedback for errors, and stamps a final calculated grade on the page—all while smartly dodging the student's original handwriting and preventing annotation collisions.
 ...
 * **Strict Problem-Based Scoring:** Groups lines of math into distinct problems. The final grade requires the *final answer* to be correct (wrong result = 0 points for that problem), while still providing targeted line-by-line visual feedback for intermediate mistakes.
...
## 🧠 How it Works Under the Hood
 ...
 2. **Prompting (Nested Logic):** It sends the full image to Gemini 3.5 Flash with a strict prompt, forcing the AI to group math into distinct problems. The AI returns a JSON array of problems containing problem_final_correct (boolean), along with a nested array of lines containing is_correct (boolean), feedback (short string), and box_2d (normalized coordinates).
 ...
 5. **Phase 3 (Smart Routing):** It evaluates incorrect steps and draws text feedback boxes. Using a radial search algorithm, the notes dynamically look for empty space nearby, gracefully dodging the stamp, the margin marks, the original equations, and other notes!
## 📄 License
